...
export default function PointeuseView({ timeclock }: { timeclock: Timeclock[] }) {
  ...
  const myOpen = timeclock.find((t) => t.member_id === member.id && !t.end)
  const liveMin = myOpen ? diffMin(myOpen.start || '00:00', nowHM()) : 0

  const sorted = [...timeclock].sort((a, b) => (b.end ? 0 : 1) - (a.end ? 0 : 1))
  const onDuty = timeclock.filter((t) => !t.end)
  const totalToday = timeclock.filter((t) => t.date === todayDate() && t.end).reduce((s, t) => s + t.minutes, 0)
          ...
          <div className="ptc-left">
            ...
            <div>
              ...
              <div className="ptc-state">
                {myOpen ? <span className="ptc-live"><span className="ptc-dot"></span> En service depuis {myOpen.start} · {fmtDur(liveMin)}</span> : 'Hors service'}
              </div>
            </div>
          </div>
          {myOpen ? (
          ...
          <tbody>
            {sorted.map((t) => (
              <tr key={t.id}>
                ...
                <td style={{ color: 'var(--ink-300)' }}>{t.date}</td>
                <td>{t.start}</td>
                <td>{t.end || <span className="badge ok" style={{ padding: '2px 8px' }}><span className="b-dot"></span> en cours</span>}</td>
                <td style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 600, color: t.end ? 'var(--gold-300)' : 'var(--ink-500)' }}>{t.end ? fmtDur(t.minutes) : '—'}</td>
                {isAdmin && (
  ...
  return (
    <Modal onClose={onClose} title="Modifier les horaires" icon={<Icons.clock size={20} />}>
      <div className="editor-panel">
        <div style={{ fontSize: 13, color: 'var(--ink-300)', marginBottom: 14 }}>{row.name} · {row.date}</div>
