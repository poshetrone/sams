...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [poles, mutuelles, gradePrimes] = await Promise.all([
    apiGet<PoleRow[]>('/poles').then((r) => r ?? []),
    apiGet<MutuelleRow[]>('/mutuelles').then((r) => r ?? []),
    apiGet<GradePrimeRow[]>('/grade-primes').then((r) => r ?? []),
  ])
  ...
  return (
    <AppShell member={current} reqCount={me.reqCount} perms={me.perms} poles={poles} mutuelles={mutuelles} gradePrimes={gradePrimes}>
      {children}
    </AppShell>
  )
}
